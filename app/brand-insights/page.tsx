  ...
  return (
    <AppShell>
      <div style={{ padding: "2rem", maxWidth: 1100, margin: "0 auto" }}>
        <PageHeader title="Client Intelligence" subtitle="Know how a client behaves in agreement negotiations before you sign — based on aggregated, anonymized analysis data." />

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: "1.5rem" }}>
          <StatCard icon="ti-thumb-up" label="Fair client ratio" value={`${fairRatio}%`} color="emerald" />
          <StatCard icon="ti-alert-triangle" label="Clients on watchlist" value={riskyCount} color="red" />
          <StatCard icon="ti-calendar-time" label="Avg usage window" value="4–9 mo" color="violet" />
          <StatCard icon="ti-building-store" label="Clients tracked" value={BRAND_RECORDS.length} color="amber" />
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 16, marginBottom: "1.5rem" }}>
          <GlassCard>
            <div style={{ fontSize: 13.5, fontWeight: 500, color: "var(--text)", marginBottom: 14 }}>Projects Tracked by Client</div>
          ...
          <GlassCard>
            <div style={{ fontSize: 13.5, fontWeight: 500, color: "var(--text)", marginBottom: 14 }}>Common Agreement Risks</div>
        ...
        <GlassCard style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "1.25rem 1.25rem 0.75rem", fontSize: 13.5, fontWeight: 500, color: "var(--text)" }}>Client Intelligence Leaderboard</div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderTop: "0.5px solid var(--glass-border)", borderBottom: "0.5px solid var(--glass-border)" }}>
                {["Client", "Risk level", "Avg usage window", "Projects tracked", "Common issues"].map((h) => (
        ...
        <p style={{ fontSize: 12, color: "var(--text3)", textAlign: "center", marginTop: "1.25rem" }}>
          Client insights are based on illustrative sample data for this demo.
        </p>
      </div>
    </AppShell>
  );
